function mainController($scope, $window, $http) {
  ...
  $scope.bodyInit = function() {
    ...
    var response = get_list_from_server($http, {
      cmd: 'LIST_EVENT_RECORD',
      params: {
      }
    }, function(response) {
      $scope.list_event_record = response.data.list_params;;
      $scope.map_list_data = make_map_from_list($scope.list_event_record,'evr_uid');
      console.log($scope.list_event_record);
