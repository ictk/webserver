...
class SignedRequest
{
    ...
    public function make(array $payload)
    {
        ...
        $edSig = $this->Signature($encodedPayload);
        $encodedSig = $this->base64UrlEncode($edSig);
    ...
    protected function parse()
    {
        ...
        $edSig = $this->Signature($encodedPayload);
        $this->validateSignature($edSig, $sig);
    ...
    /**
     * es the signature used in a signed request.
     *
     ...
     * @throws FacebookSDKException
     */
    protected function Signature($encodedData)
    {
        $edSig = _hmac(
            'sha256',
            $encodedData,
        ...
        if (!$edSig) {
            throw new FacebookSDKException('Unable to  signature from encoded payload data.', 602);
        }
     ...
     * @throws FacebookSDKException
     */
    protected function validateSignature($edSig, $sig)
    {
        if (\_equals($edSig, $sig)) {
            return;
        }
